...
tab1, tab2, tab3, tab4, tab5, tab6, tab7 = st.tabs([
    ...
    "Peak Detection", "Peak Detection (BPF)", "Time-Domain HRV Features", "EMD Analysis"
])
...
# emd
with tab7:
    st.header("Empirical Mode Decomposition (EMD)")
    
    if st.session_state.data_loaded:
        from emd_functions import (emd, calculate_respiratory_rate_from_imf, 
                                   select_respiratory_imf, analyze_imf,
        ...
        if 'imfs_emd' not in st.session_state:
            ...
            st.session_state.residual_emd = None
            st.session_state.imf_analyses = None
        
        col1, col2 = st.columns([1, 4])
        with col1:
            if st.button("Compute EMD", type="primary", use_container_width=True):
                with st.spinner("Running EMD decomposition..."):
                    imfs, residual = emd(
                        st.session_state.downsampled_signal,
                        max_imfs=max_imfs_emd,
                        max_iter=max_iter_emd,
                        sd_threshold=sd_threshold_emd
                    )
        ...
        if st.session_state.imfs_emd is not None:
            ...
            with col2:
                st.metric("Signal Length", len(st.session_state.downsampled_signal))
            with col3:
                st.metric("Sampling Rate", f"{st.session_state.downsampled_fs:.2f} Hz")
            with col4:
            ...
            for i, (imf, analysis) in enumerate(zip(imfs, imf_analyses)):
                with st.expander(f"**IMF {i+1}** - Dominant Frequency: **{analysis['dominant_freq']:.4f} Hz**", expanded=(i==0)):
                    ...
                    fig_env.add_trace(go.Scatter(
                        x=st.session_state.downsampled_t,
                        y=imf,
                        mode='lines',
                        name=f'IMF {i+1}',
                        line=dict(color='blue', width=2)
                    ...
                    if len(analysis['maxima']) > 0:
                        fig_env.add_trace(go.Scatter(
                            x=st.session_state.downsampled_t[analysis['maxima']],
                            y=imf[analysis['maxima']],
                            mode='markers',
                            name='Maxima',
                    ...
                    if len(analysis['minima']) > 0:
                        fig_env.add_trace(go.Scatter(
                            x=st.session_state.downsampled_t[analysis['minima']],
                            y=imf[analysis['minima']],
                            mode='markers',
                            name='Minima',
                    ...
                    st.markdown("---")
                    st.markdown(f"#### Frequency Spectrum (FFT)")
                    
                    fig_fft = go.Figure()
                    fig_fft.add_trace(go.Scatter(
                        x=analysis['freq_spectrum'],
                        y=analysis['magnitude_spectrum'],
                        mode='lines',
                        name='Magnitude',
                    ...
                    fig_fft.add_vline(
                        x=analysis['dominant_freq'],
                        line_dash="dash",
                        line_color="red",
                        annotation_text=f"Dominant: {analysis['dominant_freq']:.4f} Hz",
                        annotation_position="top"
                    )
                    
                    fig_fft.add_vrect(
                        x0=min_resp_freq, x1=max_resp_freq,
                        fillcolor="lightgreen", opacity=0.2,
                        layer="below", line_width=0,
                        annotation_text="Respiratory",
                    ...
                    if min_resp_freq <= dominant_freq <= max_resp_freq:
                        ...
                        st.success(f"**Respiratory Rate (f_peak × 60)**: {rr_bpm:.2f} BPM (f_peak = {dominant_freq:.3f} Hz)")
                    else:
                        # Show warning if outside range
                        st.warning(f"Dominant frequency ({dominant_freq:.3f} Hz) is outside respiratory range ({min_resp_freq}-{max_resp_freq} Hz)")
            ...
            best_imf_idx, auto_rr = select_respiratory_imf(
                imfs, fs
            )
            ...
            st.info("Click 'Compute EMD' to start the decomposition analysis")
