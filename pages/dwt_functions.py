...
def dirac(k):
    """Dirac delta function: returns 1 if k=0, else 0"""
    return 1.0 if k == 0 else 0.0

def compute_dwt_scale_features(signal, fs, max_scale=8):
    """Compute DWT scale features using Dirac delta functions"""
    ...
    # Initialize w2fb arrays
    for j in range(1, max_scale + 1):
        w2fb[j] = np.zeros(N + 1000)
    
    # Compute filter coefficients for each scale
    for j in range(1, max_scale + 1):
    ...
    # Trim the results to original signal length
    for j in range(1, max_scale + 1):
        if j in w2fb:
            w2fb[j] = w2fb[j][:N]
...
def compute_frequency_response(fs, max_scale=8):
    """Compute frequency response for DWT filters"""
